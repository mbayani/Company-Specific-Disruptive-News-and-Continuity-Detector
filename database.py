...
class Database:
    ...
    def insert_analyzed(self, processed_doc_id, upproc_docid):
        ...
        docid = self.db_analyzed.insert({'unproc_doc_id': upproc_docid, 'proc_doc_id': processed_doc_id, 'published': rec["published"], 'title': rec["title"],  'topic': '',  'severity': '', 'sentiments': '', 'text': rec["text"] })
        return docid
    ...
    def update_anlyzed(self, documnet_id, severity, sentiments, topic):
        self.db_analyzed.update({'topic': topic, 'severity': str(severity), 'sentiments': str(sentiments)}, doc_ids=[documnet_id])
