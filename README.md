...
There are 2 modes to run it (driven by settings)- 1) Batch Mode and 2) Interactive Mode
...
2) **Interactive Mode** - We can run the program to process live news feed. Program will monitor for any new news dropped in an input folder. As soon as news is dropped in a folder, it will be processed, stock process are pulled to show the effect of news.
...
### Test with unseen news Feeds
Test data to test the unseen news are available in the “repository folder”. In batch mode the system pickup these files in the same order and process one by one

![LDA Topics](docs/testdata_unseen.JPG)

**Analysis output for news will be displayed on the terminal window**
